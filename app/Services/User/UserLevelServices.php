 ...
 */
class UserLevelServices extends Service
{
    /**
    ...
    public function setUserLevel(int $uid, int $level_id, $vipinfo = [])
    {
        ...
        /** @var UserBillServices $userBillServices */
        $userBillServices = app(UserBillServices::class);
        if (!$userBillServices->save($bill_data)) {
            throw new AdminException(100006);
        }
    ...
    {
        if ($id) {
            $vipinfo = app(SystemUserLevelServices::class)->getlevel($id);
        ...
        $field[] = Form::frameImage('icon', '图标', url('admin/widget.images/index', ['fodder' => 'icon']), isset($vipinfo) ? $vipinfo->icon : '')->icon('ios-add')->width('950px')->height('505px')->modal(['footer-hide' => true]);
        $field[] =
            Form::frameImage('image', '用户等级背景', url('admin/widget.images/index', ['fodder' => 'image']), isset($vipinfo) ? $vipinfo->image : '')->icon('ios-add')->width('950px')->height('505px')->modal(['footer-hide' => true]);
        ...
        return create_form($msg, $field, url('/user/user_level'), 'POST');
    }
    ...
    public function save(int $id, array $data)
    {
        ...
        $systemUserLevel = app(SystemUserLevelServices::class);
    ...
    public function detection(int $uid)
    {
        ...
        if (!$user) {
            throw new AdminException(410284);
        }
        /** @var SystemUserLevelServices $systemUserLevel */
        $systemUserLevel = app(SystemUserLevelServices::class);
        ...
        foreach ($userAllLevel as $vipinfo) {
            ...
            if ($uservip) {
                ...
                if (!$this->dao->update($uservip['id'], $data, 'id')) {
                    throw new AdminException(410285);
                }
            } else {
                $data = array_merge($data, [
                    'is_forever' => $vipinfo['is_forever'],
    ...
    public function getUserLevelInfo(int $uid)
    {
        ...
        $userSignServices = app(UserSignServices::class);
