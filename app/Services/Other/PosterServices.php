...
class PosterServices
{
     ...
     * @throws \Exception
     */
    public static function setShareMarketingPoster($data = [], $path)
    {
        $config = [
            'text' => [
                [
     ...
     * @throws \Exception
     */
    public static function setSharePoster($config = [], $path, $name = '')
    {
        $imageDefault = [
            'left' => 0,
            'top' => 0,
    ...
    public static function remoteImage($url)
    {
        ...
        if (is_array($result)) {
            return ['status' => false, 'msg' => $result['errcode'] . '---' . $result['errmsg']];
        }
