...
class UserWechatUserDao extends BaseDao
{
    ...
    {
        return $this->searchWhere($where)->groupBy($this->alias . '.uid')->count();
    }
    ...
    public function getListByModel(array $where, string $field, string $order, int $page, int $limit): array
    {
        return $this->searchWhere($where)
            ...
            ->offset(($page - 1) * $limit)
            ->limit($limit)
            ->groupBy($this->alias . '.uid')
            ->orderByRaw(($order ? $order . ' ,' : '') . $this->alias . '.uid desc')
            ->get()
            ->toArray();
    }
     ...
     */
    public function searchWhere($where, ?array $field = []): \Illuminate\Database\Eloquent\Builder
    {
        ...
        if (isset($where['label_id']) && $where['label_id']) {
            $model = $model->whereIn($userAlias . 'uid', function ($query) use ($where) {
                if (is_array($where['label_id'])) {
                    return \DB::table('user_label_relation')->whereIn('label_id', $where['label_id'])->select('uid')->get();
                } else {
                    if (strpos($where['label_id'], ',') !== false) {
                        return \DB::table('user_label_relation')->whereIn('label_id', explode(',', $where['label_id']))->select('uid')->get();
                    } else {
                        return \DB::table('user_label_relation')->where('label_id', $where['label_id'])->select('uid')->get();
                    }
                }
            });
        }
        ...
        return $field ? $model->select($field) : $model;
    }
    ...
    public function getSex($time, $userType)
    {
        ...
        })->select('count(' . $this->alias . '.uid) as value,' . $this->join_alis . '.sex as name')
            ->groupBy($this->join_alis . '.sex')->get()->toArray();
    }
}
