 ...
 */
class QrcodeServices extends Service
{
    /**
    ...
    public function getTemporaryQrcode($type, $id)
    {
        ...
        if ($res) {
            ...
            if (empty($res['expire_seconds']) || $res['expire_seconds'] < time()) {
                $this->createTemporaryQrcode($id, $type, $res['id']);
                $res = $this->getTemporaryQrcode($type, $id);
            }
        }
        if (!$res['ticket']) {
            throw new AdminException(400552);
        }
    ...
    public function getForeverQrcode($type, $id)
    {
        ...
        if (!$res['ticket']) {
            throw new AdminException(400553);
        }
        ...
        try {
            ...
            if (!$imageInfo) {
                ...
                if ($imageInfo['image_type'] == 1) {
                    $remoteImage = PosterServices::remoteImage($siteUrl . $imageInfo['dir']);
                } else {
                    $remoteImage = PosterServices::remoteImage($imageInfo['dir']);
                }
     ...
     * @return bool
     */
    public function setQrcodeFind($id = 0, $data = [])
    {
        if (!$id) {
            return false;
        }
