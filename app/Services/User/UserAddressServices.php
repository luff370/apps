 ...
 */
class UserAddressServices extends Service
{
    /**
    ...
    public function setDefault(int $uid, int $id)
    {
        if (!$this->getAddress($id)) {
            throw new AdminException(400648);
        }
        if (!$this->dao->update($uid, ['is_default' => 0], 'uid')) {
            throw new AdminException(400649);
        }
        if (!$this->dao->update($id, ['is_default' => 1])) {
            throw new AdminException(400650);
        }
    ...
    public function editAddress(int $uid, array $addressInfo)
    {
        if ($addressInfo['id'] == 0) {
            ...
            if ($res) {
                throw new AdminException(400651);
            }
        }
        ...
        if ($addressInfo['type'] == 1 && !$addressInfo['id']) {
            ...
            $systemCity = app(SystemCityServices::class);
            ...
            if ($cityInfo && $cityInfo['city_id']) {
                ...
                if (!$cityInfo) {
                    throw new AdminException(400652);
                }
                $addressInfo['address']['city_id'] = $cityInfo['city_id'];
            }
        }
        if (!isset($addressInfo['address']['city_id']) || $addressInfo['address']['city_id'] == 0) {
            throw new AdminException(100022);
        }
