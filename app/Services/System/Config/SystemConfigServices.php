...
use App\Support\Services\FileService;
 ...
 */
class SystemConfigServices extends Service
{
    /**
     * form表单句柄
     *
     ...
     * @return mixed
     */
    public function getConfigValue(string $configName, $default = '')
    {
    ...
    public function getReadList(int $tabId)
    {
        ...
        foreach ($info as $k => $v) {
            ...
            if ($v['type'] == 'upload' && !empty($v['value'])) {
                if ($v['upload_type'] == 1 || $v['upload_type'] == 3) {
                    $info[$k]['value'] = explode(',', $v['value']);
                }
            }
        }
    ...
    public function createTextForm(string $type, array $data)
    {
        ...
        switch ($type) {
            case 'input':
                $formBuilder[] = $this->builder->input($data['menu_name'], $data['info'], $data['value'])->info($data['desc'])->placeholder($data['desc'])->col(13);
                break;
            case 'number':
                $data['value'] = isset($data['value']) ? json_decode($data['value'], true) : 0;
                $formBuilder[] = $this->builder->number($data['menu_name'], $data['info'], (float)$data['value'])->info($data['desc']);
                break;
            case 'dateTime':
                $formBuilder[] = $this->builder->dateTime($data['menu_name'], $data['info'], $data['value'])->info($data['desc']);
                break;
            case 'color':
                $data['value'] = isset($data['value']) ? json_decode($data['value'], true) : '';
                $formBuilder[] = $this->builder->color($data['menu_name'], $data['info'], $data['value'])->info($data['desc']);
                break;
        }
    ...
    public function createTextareaForm(array $data)
    {
        ...
        $formBuilder[] = $this->builder->textarea($data['menu_name'], $data['info'], $data['value'])->placeholder($data['desc'])->info($data['desc'])->rows(6)->col(13);
     ...
     * @return array
     */
    public function createRadioForm($data, $control = [], $control_two = []): array
    {
        $form = [];
        ...
        if ($parameter) {
            ...
            $form[] = $radio = $this->builder->radio($data['menu_name'], $data['info'], (int)$data['value'])->options($options)->info($data['desc'])->col(13);
            if ($control) {
     ...
     * @return array
     */
    public function createUploadForm(int $type, $data): array
    {
        $formBuilder = [];
        switch ($type) {
            case 1:
                ...
                $formBuilder[] = $this->builder->frameImage($data['menu_name'], $data['info'], '/admin/widget.images/index.html?fodder=' . $data['menu_name'], $data['value'])
            ...
            case 2:
                ...
                $formBuilder[] =
                    $this->builder->frameImages($data['menu_name'], $data['info'], '/admin/widget.images/index.html?' . http_build_query(['fodder' => $data['menu_name'], 'type' => 'many', 'maxLength' => 5]), $data['value'])
            ...
            case 3:
                ...
                $formBuilder[] = $this->builder->uploadFile($data['menu_name'], $data['info'], url('/admin/file/upload/1', ['type' => 1]), $data['value'])
    ...
    public function createCheckboxForm(array $data)
    {
        ...
        if ($parameter) {
            ...
            $formBuilder[] = $this->builder->checkbox($data['menu_name'], $data['info'], $data['value'])->options($options)->info($data['desc'])->col(13);
        }
    ...
    public function createSelectForm(array $data)
    {
        ...
        if ($parameter) {
            ...
            $formBuilder[] = $this->builder->select($data['menu_name'], $data['info'], $data['value'])->options($options)->info($data['desc'])->col(13);
        }
    ...
    {
        $data['value'] = json_decode($data['value'], true) ?: '';
        $formBuilder[] = $this->builder->color($data['menu_name'], $data['info'], $data['value'])->info($data['desc'])->col(13);
    ...
    public function createConfigForm(int $tabId, array $relatedRule)
    {
        ...
        if (!$relatedRule) {
            $formBuilder = $this->createNoCrontrolForm($list);
        } else {
            $formBuilder = $this->createBindCrontrolForm($list, $relatedRule);
        }
     ...
     * @throws \FormBuilder\Exception\FormBuilderException
     */
    public function createForm($appId, array $list): array
    {
        if (!$list) {
            return [];
        }
        ...
        $formBuilder[] = $this->builder->hidden('app_id', $appId);
        ...
        foreach ($list as $key => $data) {
            ...
            switch ($data['type']) {
                case 'text'://文本框
                    $formBuilder = array_merge($formBuilder, $this->createTextForm($data['input_type'], $data));
                    break;
                case 'radio'://单选框
                    $builder = [];
                                ...
                                if (isset($role['son_type'][$sk . '@'])) {
                                    ...
                                    if (isset($son_type_two['son_type'])) {
                                        foreach ($son_type_two['son_type'] as $ssk => $ssv) {
                                            if (isset($list[$ssk]['menu_name']) && $list[$ssk]['menu_name'] == 'watermark_text_color') {
                                                $list[$ssk]['type'] = 'color';
                                            }
                    ...
                    $formBuilder = array_merge($formBuilder, $this->createRadioForm($data, $builder, $builder_two));
                    break;
                case 'textarea'://多行文本框
                    $formBuilder = array_merge($formBuilder, $this->createTextareaForm($data));
                    break;
                case 'upload'://文件上传
                    $formBuilder = array_merge($formBuilder, $this->createUploadForm((int)$data['upload_type'], $data));
                    break;
                case 'checkbox'://多选框
                    $formBuilder = array_merge($formBuilder, $this->createCheckboxForm($data));
                    break;
                case 'select'://多选框
                    $formBuilder = array_merge($formBuilder, $this->createSelectForm($data));
                    break;
            }
        }
        ...
        foreach ($list as $key => $data) {
            switch ($data['type']) {
                case 'text'://文本框
                    ...
                    break;
                case 'radio'://单选框
                    $formBuilder = array_merge($formBuilder, $this->createRadioForm($data));
                    break;
                case 'textarea'://多行文本框
                    $formBuilder = array_merge($formBuilder, $this->createTextareaForm($data));
                    break;
            ...
            if (isset($rv['son_type'])) {
                ...
                switch ($data['type']) {
                    ...
                    case 'radio'://单选框
                        ...
                        $formBuilder = array_merge($formBuilder, $this->createRadioForm($data, $son_builder));
                        break;
                    case 'textarea'://多行文本框
                        $formBuilder = array_merge($formBuilder, $this->createTextareaForm($data));
                        break;
     ...
     * @throws \FormBuilder\Exception\FormBuilderException
     */
    public function getConfigForm($url, int $tabId): array
    {
        $configTab = SystemConfigTab::query()->findOrFail($tabId);
        $appId = $configTab['app_id'];
        $title = $configTab['title'];

        $list = $this->dao->getConfigTabAllList($tabId);
        $formBuilder = $this->createForm($appId, $list);
        $name = 'setting';
        if ($url) {
            $name = explode('/', $url)[2] ?? $name;
        }
        $postUrl = $this->postUrl[$name]['url'] ?? '/admin/setting/config/save_basics';

        return create_form($title, $formBuilder, url($postUrl), 'POST');
    }
    ...
    public function checkParam($url, $post)
    {
        ...
        if ($auth) {
            ...
            $systemConfigTabServices = app(SystemConfigTabServices::class);
     ...
     * @throws \App\Exceptions\AdminException
     */
    public function editConfigForm(int $id): array
    {
        $menu = $this->dao->getModel()->newQuery()->find($id);
        if (!$menu) {
            throw new AdminException(100026);
        }
        /** @var SystemConfigTabServices $service */
        $service = app(SystemConfigTabServices::class);
        $formBuilder = [];
        $formBuilder[] = $this->builder->input('menu_name', '字段变量', $menu['menu_name'])->disabled(1);
        $formBuilder[] = $this->builder->hidden('type', $menu['type']);
        $formBuilder[] = $this->builder->select('config_tab_id', '分类', (int)$menu['config_tab_id'])->setOptions($service->getSelectForm());
        $formBuilder[] = $this->builder->input('info', '配置名称', $menu['info'])->autofocus(1);
        $formBuilder[] = $this->builder->input('desc', '配置简介', $menu['desc']);
        switch ($menu['type']) {
            case 'text':
                $menu['value'] = json_decode($menu['value'], true);
                $formBuilder[] = $this->builder->select('input_type', '类型', $menu['input_type'])->setOptions([
                ...
                //输入框验证规则
                $formBuilder[] = $this->builder->input('value', '默认值', $menu['value']);
                if (!empty($menu['required'])) {
                    $formBuilder[] = $this->builder->number('width', '文本框宽(%)', (int)$menu['width']);
                    $formBuilder[] = $this->builder->input('required', '验证规则', $menu['required'])->placeholder('多个请用,隔开例如：required:true,url:true');
                }
                break;
            case 'textarea':
                $menu['value'] = json_decode($menu['value'], true);
                //多行文本
                if (!empty($menu['high'])) {
                    $formBuilder[] = $this->builder->textarea('value', '默认值', $menu['value'])->rows(5);
                    $formBuilder[] = $this->builder->number('width', '文本框宽(%)', (int)$menu['width']);
                    $formBuilder[] = $this->builder->number('high', '多行文本框高(%)', (int)$menu['high']);
                } else {
                    $formBuilder[] = $this->builder->input('value', '默认值', $menu['value']);
                }
                break;
            case 'radio':
                $formBuilder = array_merge($formBuilder, $this->createRadioForm($menu));
                //单选和多选参数配置
                if (!empty($menu['parameter'])) {
                    $formBuilder[] = $this->builder->textarea('parameter', '配置参数', $menu['parameter'])->placeholder("参数方式例如:\n1=>白色\n2=>红色\n3=>黑色");
                }
                break;
            case 'checkbox':
                $formBuilder = array_merge($formBuilder, $this->createCheckboxForm($menu));
                //单选和多选参数配置
                if (!empty($menu['parameter'])) {
                    $formBuilder[] = $this->builder->textarea('parameter', '配置参数', $menu['parameter'])->placeholder("参数方式例如:\n1=>白色\n2=>红色\n3=>黑色");
                }
                break;
            case 'upload':
                $formBuilder = array_merge($formBuilder, $this->createUploadForm(($menu['upload_type']), $menu));
                //上传类型选择
                if (!empty($menu['upload_type'])) {
                    $formBuilder[] = $this->builder->radio('upload_type', '上传类型', $menu['upload_type'])->options([['value' => 1, 'label' => '单图'], ['value' => 2, 'label' => '多图'], ['value' => 3, 'label' => '文件']]);
                }
                break;
        }
        $formBuilder[] = $this->builder->number('sort', '排序', (int)$menu['sort']);
        $formBuilder[] = $this->builder->radio('status', '状态', $menu['status'])->options([['value' => 1, 'label' => '显示'], ['value' => 2, 'label' => '隐藏']]);

        return create_form('编辑字段', $formBuilder, url('/admin/setting/config/' . $id), 'PUT');
    }
    ...
    public function createFormRule(int $type, int $tab_id): array
    {
        ...
        if ($form_type) {
            $formBuilder[] = $this->builder->hidden('type', $form_type);
            $formBuilder[] = $this->builder->select('config_tab_id', '分类', $tab_id)->setOptions($service->getSelectForm());
            if ($info_type) {
                $formBuilder[] = $info_type;
            }
            $formBuilder[] = $this->builder->input('info', '配置名称')->autofocus(1);
            $formBuilder[] = $this->builder->input('menu_name', '字段变量')->placeholder('例如：site_url');
            $formBuilder[] = $this->builder->input('desc', '配置简介');
            $formBuilder = array_merge($formBuilder, $parameter);
            $formBuilder[] = $this->builder->number('sort', '排序', 0);
            $formBuilder[] = $this->builder->radio('status', '状态', 1)->options($this->formStatus());
        }

        return create_form('添加字段', $formBuilder, url('/admin/setting/config'), 'POST');
    }
     ...
     * @return bool
     */
    public function validateRadioAndCheckbox($data): bool
    {
        $option = [];
        $option_new = [];
    ...
    public function checkBrokerageBinding(array $post)
    {
        try {
            ...
            if ($config_old != 2 && $config_data == 2) {
                ...
                $agentManage = app(AgentManageServices::class);
