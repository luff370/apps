...
class LangTypeServices extends Service
{
    /**
    ...
    public function langTypeForm(int $id = 0): array
    {
        ...
        return create_form($id ? '修改语言类型' : '新增语言类型', $field, url('/setting/lang_type/save/' . $id), 'POST');
    }
    ...
    public function langTypeSave(array $data)
    {
        if ($data['id']) {
            ...
            if ($res) {
                ...
                $codeServices = app(LangCodeServices::class);
        ...
        if ($data['is_default'] == 1) {
            $this->dao->update([['id', '<>', $id]], ['is_default' => 0]);
        }
    ...
    public function langTypeDel(int $id = 0)
    {
        ...
        $countryServices = app(LangCountryServices::class);
