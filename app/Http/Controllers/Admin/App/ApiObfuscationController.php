...
class ApiObfuscationController extends Controller
{
    ...
    {
        $data = $this->getMore([
            ['app_id', 0], ['package_name', ''], ['enabled', 0], ['encrypt_request', 0], ['encrypt_response', 0], ['allow_plaintext_request', 1], ['image_url_enabled', 0], ['image_path_alias_enabled', 0], ['image_domain', ''], ['api_domain', ''], ['request_key_map', []], ['response_key_map', []], ['payload_field', 'payload'], ['sign_field', 'sign'], ['timestamp_field', 'ts'], ['nonce_field', 'nonce'], ['version_field', 'ver'], ['timestamp_window_seconds', 300], ['nonce_ttl_seconds', 300], ['cipher', 'AES-256-CBC'], ['crypto_key', ''], ['crypto_iv', ''], ['crypto_sign_key', ''], ['image_fields', ''], ['image_prefixes', ''],
        ]);
    ...
    {
        $data = $this->getMore([['id', 0], ['app_id', 0], ['package_name', ''], ['interface_id', 0], ['alias', ''], ['request_key_map', []], ['response_key_map', []], ['is_enable', 1], ['remark', '']]);
    ...
    {
        $data = $this->getMore([['id', 0], ['alias', ''], ['request_key_map', []], ['response_key_map', []], ['is_enable', 1], ['remark', '']]);
    ...
    {
        $data = $this->getMore([['app_id', 0], ['package_name', ''], ['map_rule', 'short'], ['overwrite', 0]]);
