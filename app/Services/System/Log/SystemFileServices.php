...
use App\Support\Services\FileService as FileClass;
 ...
 */
class SystemFileServices extends Service
{
    /**
     * 构造方法
     * SystemFileService constructor.
     *
    ...
    public function parseToken(string $token): bool
    {
        ...
        $cacheServices = app(CacheService::class);
        ...
        //检测token是否过期
        $md5Token = md5($token);
        if (!$cacheServices->hasToken($md5Token) || !($cacheToken = $cacheServices->getTokenBucket($md5Token))) {
            throw new AuthException(110008);
        }
        ...
        try {
            $jwtAuth->verifyToken();
            $cacheServices->setTokenBucket($md5Token, $cacheToken, $cacheToken['exp']);
    ...
    public function getFileList()
    {
        ...
        $cha = [];    //不同的文件
        $len = strlen($rootPath);
        $file = $this->dao->getAllFiles();//数据库中的文件
        if (empty($file)) {
            foreach ($arr as $k => $v) {
                $update_time = stat($v);
            ...
            $res = true;
            $res = \DB::transaction(function () use ($data_num, $res) {
