 ...
 */
class UserBillServices extends Service
{
    /**
     * 用户记录模板
     *
    ...
    protected $incomeData = [
        ...
        'report_reward_give_energy' => [
            'title' => '违章举报获得正能量',
            'category' => 'integral',
            'type' => 'report_reward',
            'mark' => '违章举报审核通过，获得{%num%}正能量',
            'status' => 1,
            'pm' => 1,
        ],
        'user_withdrawal_by_energy' => [
            'title' => '申请提现使用正能量',
            'category' => 'integral',
            'type' => 'user_withdrawal',
            'mark' => '用户提现申请提交成功，使用{%num%}正能量',
            'status' => 1,
            'pm' => 0,
        ],
        'user_withdrawal_by_balance' => [
            'title' => '申请余额提现',
            'category' => 'balance',
            'type' => 'user_withdrawal',
            'mark' => '用户提现申请提交成功，使用{%num%}元',
            'status' => 1,
            'pm' => 0,
        ],
        'withdrawal_fail_refund_energy' => [
            'title' => '提现申请审核失败',
            'category' => 'integral',
            'type' => 'user_withdrawal',
            'mark' => '用户提现申请审核失败，退回{%num%}正能量',
            'status' => 1,
            'pm' => 1,
        ],
        'withdrawal_fail_refund_balance' => [
            'title' => '余额提现失败',
            'category' => 'balance',
            'type' => 'user_withdrawal',
            'mark' => '用户提现申请审核失败，退回{%num%}元',
            'status' => 1,
            'pm' => 1,
        ],
        'get_two_brokerage' => [
    ...
    public function getRechargeSum(int $uid = 0, $where_time = [])
    {
        ...
        if ($uid) {
            $where['uid'] = $where_system['uid'] = $uid;
        }
        if ($where_time) {
            $where['add_time'] = $where_system['add_time'] = $where_time;
        }
     ...
     * @return bool|mixed
     */
    public function income(string $type, int $appId, int $uid, $number, $balance, $link_id)
    {
     ...
     * @param $orderInfo
     */
    public function orderGiveIntegral($orderInfo)
    {
        return $this->transaction(function () use ($orderInfo) {
            ...
            // 用户积分余额
            $userIntegral = $userServices->value(['uid' => $orderInfo['uid']], 'integral');
            // 订单赠送积分
            $giveIntegral = $orderInfo['gain_integral'] ?? $orderInfo['pay_price'] * 10;

            // 增加用户积分余额
            $userServices->bcInc($orderInfo['uid'], 'integral', (string) $giveIntegral, 'uid');
            // 增加用户积分赠送记录
            $userBillServices->income('order_give_integral', $orderInfo['uid'], (int) $giveIntegral, $userIntegral + $giveIntegral, $orderInfo['id']);

            Log::info('订单完成赠送用积分', ['order_id' => $orderInfo['id'], 'uid' => $orderInfo['uid'], 'give_integral' => $giveIntegral]);
    ...
    public function inviteUserIncExp(int $spreadUid)
    {
        ...
        /** @var UserServices $userServices */
        $userServices = app(UserServices::class);
        $spread_user = $userServices->getUserInfo($spreadUid);
        if (!$spread_user) {
            return false;
        }
        $exp_num = sys_config('invite_user_exp', 0);
        if ($exp_num) {
            $userServices->incField($spreadUid, 'exp', (int) $exp_num);
            $data = [];
            $data['uid'] = $spreadUid;
            $data['number'] = $exp_num;
            $data['category'] = 'exp';
    ...
    public function getCommissionList(array $where, int $limit = 0)
    {
        ...
        $userUserBrokerage = app(UserUserBrokerageServices::class);
        [$count, $list] =
            $userUserBrokerage->getBrokerageList($where_data, 'b.type,b.pm,sum(IF(b.pm = 1, b.number, 0)) as income,sum(IF(b.pm = 0, b.number, 0)) as pay,u.nickname,u.phone,u.uid,u.now_money,u.brokerage_price,b.add_time as time', $order_string, $limit);
        $uids = array_unique(array_column($list, 'uid'));
        /** @var UserExtractServices $userExtract */
        $userExtract = app(UserExtractServices::class);
    ...
    public function orderRefundBrokerageBack(int $id, string $orderId)
    {
        ...
        $services = app(UserBrokerageFrozenServices::class);
    ...
    public function getUserBillList(int $uid, int $type)
    {
        ...
        switch ((int) $type) {
            ...
            case 4:
                ...
                $userExtractServices = app(UserExtractServices::class);
                $userExtract = $userExtractServices->getColumn(['uid' => $uid], 'fail_msg', 'id');
                break;
        }
        $field = 'FROM_UNIXTIME(add_time,"%Y-%m") as time,group_concat(id SEPARATOR ",") ids';
        [$page, $limit] = $this->getPageValue();
        $list = $this->dao->getUserBillListBygroupBy($where, $field, 'time', $page, $limit);
        $data = [];
        if ($list) {
            ...
            foreach ($list as $item) {
                ...
                foreach ($id as $v) {
                    if (isset($info[$v])) {
                        if ($info[$v]['pm'] == 1 && $type == 4) {
                            $info[$v]['fail_msg'] = $userExtract[$info[$v]['link_id']];
                        }
                        $value['list'][] = $info[$v];
                    }
                }
    ...
    public function spread_order(int $uid, array $data)
    {
        ...
        $storeOrderServices = app(StoreOrderServices::class);
        [$page, $limit] = $this->getPageValue();
        $time = [];
        ...
        if ($list) {
            $uids = array_unique(array_column($list, 'uid'));
            $userInfos = $userServices->getColumn([['uid', 'in', $uids]], 'uid,avatar,nickname', 'uid');
            foreach ($list as &$item) {
