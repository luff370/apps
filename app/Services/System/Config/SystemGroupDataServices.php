 ...
 */
class SystemGroupDataServices extends Service
{
    /**
     ...
     * @return array
     */
    public function getConfigNameValue(string $configName): array
    {
        /** @var SystemGroupServices $systemGroupServices */
        $systemGroupServices = app(SystemGroupServices::class);
        $value = $this->dao->getGroupDate((int) $systemGroupServices->getConfigNameId($configName));
    ...
    public function getGroupDataList(array $where)
    {
        ...
        $services = app(SystemGroupServices::class);
        ...
        foreach ($fields as $key => $value) {
            ...
            switch ($value["type"]) {
                ...
                case 'upload':
                    ...
                    $f[] =
                        Form::frameImage($value["title"], $value["name"], url('/adminadmin/widget.images/index', ['fodder' => $value["title"], 'big' => 1], true), $image)
                            ->icon('ios-image')
                            ->width('950px')
                            ->height('505px')
                ...
                case 'uploads':
                    ...
                    $f[] =
                        Form::frameImages($value["title"], $value["name"], url('/admin/widget.images/index', ['fodder' => $value["title"], 'big' => 1, 'type' => 'many', 'maxLength' => 5], true), $images)
                            ->maxLength(5)
                            ->icon('ios-images')
                            ->width('950px')
                            ->height('505px')
    ...
    {
        return create_form('添加数据', $this->createGroupForm($gid), url('/admin/setting/group_data'));
    }
    ...
    public function updateForm(int $gid, int $id)
    {
        ...
        return create_form('编辑数据', $this->createGroupForm($gid, $groupData->toArray()), url('/admin/setting/group_data/' . $id), 'PUT');
    }
    ...
    public function saveAllData(array $params, string $config_name)
    {
        ...
        \DB::transaction(function () use ($gid, $params, $fields) {
    ...
    public function checkSeckillTime(SystemGroupServices $services, $gid, $params, $id = 0)
    {
        ...
        if ($name == 'routine_seckill_time') {
            ...
            if (($params['time'] + $params['continued']) > 24) {
                throw new AdminException(400194);
            }
            ...
            foreach ($time as $v) {
                if (in_array($v, $times)) {
                    throw new AdminException(400195);
                }
            }
        }
    }
