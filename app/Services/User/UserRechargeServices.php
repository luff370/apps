 ...
 */
class UserRechargeServices extends Service
{
    /**
    ...
    public function refund_edit(int $id)
    {
        ...
        return create_form('编辑', $f, url('/finance/recharge/' . $id), 'PUT');
    }
    ...
    public function importNowMoney(int $uid, $price)
    {
        ...
        $frozenPrices = app(UserBrokerageServices::class);
        ...
        if ($price > $commissionCount) {
            throw new AdminException(400680);
        }
        $edit_data = [];
        ...
        if (!$re = $this->dao->save($rechargeInfo)) {
            throw new AdminException(400681);
        }
        ...
        switch ((int) $type) {
            case 0: //支付充值余额
                ...
                if ($recharId) {
                    ...
                    $systemGroupData = app(SystemGroupDataServices::class);
                    $data = $systemGroupData->getDateValue($recharId);
                    if ($data === false) {
                        throw new AdminException(400682);
                    } else {
                        $paid_price = $data['give_money'] ?? 0;
                    }
                }
                $recharge_data = [];
                $recharge_data['order_id'] = app(StoreOrderCreateServices::class)->getNewOrderId('cz');
                ...
                if (!$rechargeOrder = $this->dao->save($recharge_data)) {
                    throw new AdminException(400683);
                }
                try {
                    /** @var RechargeServices $recharge */
                    $recharge = app(RechargeServices::class);
    ...
    public function rechargeSuccess($orderId, array $other = [])
    {
        ...
        if (!$this->dao->update($order['id'], ['paid' => 1, 'pay_time' => time(), 'trade_no' => $other['trade_no'] ?? ''], 'id')) {
            throw new AdminException(410286);
        }
