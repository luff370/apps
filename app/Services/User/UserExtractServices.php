...
use App\Support\Services\pay\Pay;
 ...
 */
class UserExtractServices extends Service
{
    /**
    ...
    public function changeFail(int $id, $userExtract, $message)
    {
        ...
        \DB::transaction(function () use ($user, $uid, $id, $extract_number, $message, $userServices, $status, $fail_time) {
            //增加佣金记录
            /** @var UserBrokerageServices $userBrokerageServices */
            $userBrokerageServices = app(UserBrokerageServices::class);
    ...
    public function edit(int $id)
    {
        ...
        return create_form('编辑', $f, url('/finance/extract/' . $id), 'PUT');
    }
    ...
    public function bank(int $uid)
    {
        ...
        $services = app(UserBrokerageServices::class);
    ...
    public function cash(int $uid, array $data)
    {
        ...
        if ($data['money'] > $commissionCount) {
            throw new AdminException(400661);
        }

        $extractPrice = $user['brokerage_price'];
        $userExtractMinPrice = sys_config('user_extract_min_price');
        if ($data['money'] < $userExtractMinPrice) {
            throw new AdminException(400662, ['money' => $userExtractMinPrice]);
        }
        if ($extractPrice < 0) {
            throw new AdminException(400663, ['money' => $data['money']]);
        }
        if ($data['money'] > $extractPrice) {
            throw new AdminException(400663, ['money' => $data['money']]);
        }
        if ($data['money'] <= 0) {
            throw new AdminException(400664);
        }
        $openid = '';
        $insertData = [
            'uid' => $user['uid'],
            ...
            if ($data['extract_type'] == 'bank') {
                ...
                if ($data['extract_type'] == 'weixin') {
                    ...
                    if (sys_config('brokerage_type', 0) && $openid) {
                        if ($data['money'] < 1) {
                            throw new AdminException(400665);
                        }
                    }
                }
            }
        }
        $res1 = \DB::transaction(function () use ($insertData, $data, $uid, $userServices, $user, $mark) {
            if (!$res1 = $this->dao->save($insertData)) {
                throw new AdminException(410121);
            }
            $balance = bcsub((string) $user['brokerage_price'], (string) $data['money'], 2) ?? 0;
            if (!$userServices->update($uid, ['brokerage_price' => $balance], 'uid')) {
                throw new AdminException(410121);
            }
            ...
            if ($insertData['extract_type'] == 'weixin' && $insertData['status'] == 1 && isset($insertData['wechat_order_id'])) {
                $res = WechatServices::merchantPay($openid, $insertData['wechat_order_id'], $data['money'], '提现佣金到零钱');
                if (!$res) {
                    throw new AdminException(400658);
                }
                /** @var CapitalFlowServices $capitalFlowServices */
        ...
        try {
            ChannelServices::instance()->send('WITHDRAW', ['id' => $res1->id]);
        } catch (\Exception $e) {
        }
        /** @var SystemAdminServices $systemAdmin */
        $systemAdmin = app(SystemAdminServices::class);
