...
use App\Support\Utils\fileVerification;
 ...
 */
class UpgradeServices extends Service
{
    ...
    public function __construct(UpgradeLogDao $dao)
    {
        ...
        if ($versionData['version_code'] < 450) {
            return true;
        }
    ...
    public function getVersion(string $name = '', string $path = '')
    {
        ...
        foreach ($list as $val) {
            [$k, $v] = explode('=', str_replace(PHP_EOL, '', $val));
            $arr[$k] = $v;
        }
    ...
    public function download(string $seq, string $url, string $downloadPath, string $fileName, int $timeout = 300)
    {
        ...
        /** @var FileService $fileServices */
        $fileServices = app(FileService::class);
        $downloadFilePath = $downloadPath . DS . substr($fileName, 0, strpos($fileName, 'zip') - 1);
        if (!$fileServices->extractFile($downloadPath . DS . $fileName, $downloadFilePath)) {
    ...
    public function databaseBackup($token): bool
    {
        try {
            ...
            $backServices = app(SystemDatabackupServices::class);
        ...
        try {
            ...
            foreach ($projectPath as $key => $path) {
                foreach ($path as $item) {
                    if ($key == 'file') {
                        $fileServices->handleFile($appPath . $item, $backupDir . DS . $item, 'copy', false, ['zip']);
                    } else {
                        $fileServices->handleDir($appPath . $item, $backupDir . DS . $item, 'copy', false, ['uploads']);
                    }
                }
            }
            ...
            /** @var FileService $fileServices */
            $fileServices = app(FileService::class);
            $result = $fileServices->addZip($backupDir, $filePath, $backupDir);
            if (!$result) {
    ...
    public function getProjectDir($path): array
    {
        /** @var FileService $fileServices */
        $fileServices = app(FileService::class);
        $list = $fileServices->getDirs($path);
    ...
    public function checkSignature()
    {
        ...
        $verification = app(fileVerification::class);
    ...
    public function coverageProject(string $token): bool
    {
        ...
        if (is_file($serverPackageName) && !$fileServices->extractFile($serverPackageName, app()->getRootPath())) {
            throw new AdminException('服务端解压失败');
        }

        if (is_file($clientPackageName) && !$fileServices->extractFile($clientPackageName, app()->getRootPath())) {
            throw new AdminException('客户端解压失败');
        }

        if (is_file($pcPackageName) && !$fileServices->extractFile($pcPackageName, app()->getRootPath())) {
