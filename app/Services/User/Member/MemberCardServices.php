...
use App\Support\Services\SystemConfigServices;
use App\Services\Order\StoreOrderCreateServices;

class MemberCardServices extends Service
{
    /** 初始化，获得dao层句柄
    ...
    public function getSearchList(array $where = [])
    {
        ...
        if ($list) {
            foreach ($list as $k => $v) {
                if ($v['use_uid']) {
                    $user_info = $userServices->getUserInfo($v['use_uid'], ['nickname', 'phone', 'real_name']);
    ...
    public function drawMemberCard(array $data, int $uid)
    {
        ...
        if (!$isOpenMember) {
            throw new AdminException(400621);
        }
        if (!isset($data['member_card_code']) || !$data['member_card_code']) {
            throw new AdminException(400622);
        }
        if (!isset($data['member_card_code']) || !$data['member_card_pwd']) {
            throw new AdminException(400623);
        }
        $card_info = $this->dao->getOneByWhere(['card_number' => trim($data['member_card_code'])]);
        if (!$card_info) {
            throw new AdminException(400624);
        }
        /** @var MemberCardBatchServices $memberBatchServices */
        $memberBatchServices = app(MemberCardBatchServices::class);
        $batch_info = $memberBatchServices->getOne($card_info['card_batch_id']);
        if (!$batch_info) {
            throw new AdminException(400625);
        }
        ...
        if ($card_info['card_password'] != trim($data['member_card_pwd'])) {
            throw new AdminException(400626);
        }
        if ($card_info['use_uid'] && $card_info['use_time']) {
            throw new AdminException(400627);
        }
        /** @var UserServices $userServices */
        $userServices = app(UserServices::class);
        ...
        if ($user_info->is_money_level > 0 && $user_info->is_ever_level == 1) {
            throw new AdminException(400628);
        }
        ...
        \DB::transaction(function () use ($card_info, $user_info, $batch_info, $memberBatchServices, $userServices, $data) {
            ...
            if ($res1) {
                ...
                $OtherOrderServices = app(OtherOrderServices::class);
                ...
                $record_data['order_id'] = $storeOrderCreateServices->getNewOrderId();
    ...
    public function checkmemberType(string $member_type)
    {
        ...
        if (!array_key_exists($member_type, $member_type_arr)) {
            throw new AdminException(400629);
        }
    ...
    public function getMemberRightsInfo()
    {
        ...
        $memberRight = $memberRightServices->getSearchList(['status' => 1]);
        if ($memberRight['list']) {
    ...
    public function getMemberTypeInfo()
    {
        ...
        $systemConfigServices = app(SystemConfigServices::class);
        $data = [];
        foreach (self::$_memberTypePrefix as $v) {
            $data[$v] = $systemConfigServices::more([$v . '_title', $v . '_vip_day', $v . '_pre_price', $v . '_price']);
        }
    ...
    public function DoMemberType()
    {
        ...
        $list = $memberShipServices->getApiList(['is_del' => 0]);
        foreach ($list as $v) {
            $data[] = [
                'mc_id' => $v['id'],
                'title' => $v['title'],
    ...
    public function getExportData($where)
    {
        ...
        /** @var UserServices $userServices */
        $userServices = app(UserServices::class);
        /** @var MemberCardBatchServices $batchServices */
        $batchServices = app(MemberCardBatchServices::class);
        foreach ($data as $k => $v) {
            $data[$k]['use_time'] = $v['use_time'] != 0 ? date('Y-m-d H:i:s', $v['use_time']) : "";
            $data[$k]['user_name'] = '';
            $data[$k]['user_phone'] = '';
            if ($v['use_uid'] != 0) {
                $userInfo = $userServices->get($v['use_uid']);
                $data[$k]['user_name'] = $userInfo['nickname'] ?: $userInfo['account'];
                $data[$k]['user_phone'] = $userInfo['phone'];
            }
        }
        $batchInfo = $batchServices->getOne($where['batch_card_id']);
    ...
    public function getSearchRecordList(array $where)
    {
        ...
        $otherOrderSevice = app(OtherOrderServices::class);
    ...
    public function isOpenMemberCard(string $rightType = '', bool $get_number = true)
    {
        ...
        if (!$rightType) {
            ...
            /** @var MemberRightServices $memberRightServices */
            $memberRightServices = app(MemberRightServices::class);
            $memberRight = $memberRightServices->getOne(['right_type' => $rightType], 'status,number');
